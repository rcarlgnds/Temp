...
interface LobbyViewProps {
  ...
  onStartGame: (roomId: string, ws: WebSocket | null) => Promise<void>;
...
export function LobbyView({
  ...
  const GAME_URL = process.env.NEXT_PUBLIC_GAME_URL;
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.message === "update room" && room?.id) {
          ...
          onRefreshLobby(room.id);
        } else if (data.eventType === "start-game" && room?.id) {
          setIsStarting(true);

          const currentUser = room.players.find(p => p.playerId === session.user.id);
          if (currentUser && currentUser.playerCode) {
            navigator.clipboard.writeText(currentUser.playerCode);
            notifications.show({
              title: 'Player Code Copied!',
              message: 'Your personal code has been copied to your clipboard.',
              color: 'teal',
              icon: <IconCheck />,
            });
          }
          ...
          const interval = setInterval(() => {
            ...
            if (timer <= 0) {
              ...
              if (GAME_URL) {
                ...
                console.error("NEXT_PUBLIC_GAME_URL is not defined!");
              }
            }
          }, 1000);
        }
  ...
  }, [room, session?.user?.id, onRefreshLobby]);
  ...
  return (
      <>
        <Modal opened={isStarting} onClose={() => {}} withCloseButton={false} centered size="md" closeOnClickOutside={false} closeOnEscape={false}>
          <Stack align="center" gap="lg" p="xl">
            <Title order={2}>Game Starting!</Title>
            <Text>Your player code has been copied to the clipboard.</Text>
            <Text size="xl" fw={700}>Redirecting in {countdown}...</Text>
            <Loader color="yellow" />
          </Stack>
        </Modal>
              ...
              <Stack gap="xs" mt="sm" ml="md">
                <Group justify="space-between">
                  <Group gap="xs" c="dimmed">
                    ...
                    <Text size="xs">Host ID: {room.hostName}</Text>
                  </Group>
                  <Badge
